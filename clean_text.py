...
# own
from trafilatura.external import justext_rescue, try_readability
from trafilatura.filters import duplicate_test, language_filter, put_in_cache
from trafilatura.htmlprocessing import (convert_tags, discard_unwanted,
                             discard_unwanted_comments, handle_textnode,
                             manual_cleaning, process_node, prune_html)
from trafilatura.metadata import extract_metadata
from trafilatura.settings import (HTML_CLEANER, MIN_EXTRACTED_SIZE, MIN_EXTRACTED_COMM_SIZE,
                       MIN_OUTPUT_SIZE, MIN_OUTPUT_COMM_SIZE, TAG_CATALOG)
from trafilatura.utils import load_html, sanitize, trim
from trafilatura.xml import add_xml_meta, build_tei_output, build_xml_output, validate_tei, xmltotxt
from trafilatura.xpaths import BODY_XPATH, COMMENTS_XPATH
...
def handle_table(table_elem):
    ...
    for subelement in table_elem.iter():
        print("newrow:",subelement.tag)
        i += 1
        if subelement.tag in ('p'):
            subelement.tag='td'
            print(subelement)
        if subelement.tag in ('item','list','a'):
            subelement.tag='td'
        if subelement.tag == 'tr':
            ...
            #textcontent = ''.join(subelement.itertext())
                ...
                #continue
        elif subelement.tag in ('td','p','th','list','item','a'):
            # process
...
def recover_wild_paragraphs(tree, result_body, potential_tags=TAG_CATALOG):
    ...
    for element in search_tree.iter('blockquote', 'code', 'p', 'pre', 'q', 'quote','head','list'): # 'head', 'list'
...
def extract_content(tree, include_tables=True):
...
def extract(filecontent, url=None, record_id='0001', no_fallback=False,
            include_comments=False, csv_output=False, xml_output=False,
